    ...
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-md bg-[#111] border border-zinc-800 p-8 rounded-2xl shadow-2xl">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white mb-2">Initialize Your Vault</h1>
          <p className="text-zinc-400 text-sm">
            This password encrypts your data locally. <br/>
            <span className="text-red-400">It cannot be recovered if lost.</span>
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1">Master Password</label>
            <input
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="w-full bg-black border border-zinc-800 rounded-lg py-3 px-4 focus:outline-none focus:border-blue-500 transition-colors"
          ...
          <button
            type="submit"
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg shadow-lg transition-all active:scale-[0.98] mt-4"
          >
            Create Secure Vault
          </button>
        </form>
